  ...
  Taxon.class_eval do
    def products_by_zone(product_zone)
      ...
      if flow_experience_key.present?
        products_by_experience(flow_experience_key)
      else
        products
      end
    end

    def products_by_experience(flow_experience_key)
      ...
      query = <<~SQL
          SELECT DISTINCT spree_products.* FROM spree_products 
            INNER JOIN spree_variants ON spree_variants.product_id = spree_products.id AND 
              spree_variants.is_master = 'f' AND spree_variants.deleted_at IS NULL 
            INNER JOIN (
